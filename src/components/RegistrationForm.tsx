...
export function RegistrationForm({ onClose, onSubmit }: RegistrationFormProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-black/90 backdrop-blur-xl rounded-2xl border border-white/10 p-4 w-[420px] mx-auto"
    >
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white bg-gradient-to-r from-white to-purple-200 bg-clip-text text-transparent">Complete Your Profile</h2>
        <motion.button 
          onClick={onClose} 
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-white/80 mb-1.5 block">Username *</label>
            <input
              required
              type="text"
          ...
          <div className="relative">
            <label className="text-sm text-white/80 mb-1.5 block">Profession *</label>
            <select
              required
              value={formData.profession}
              onChange={e => setFormData(prev => ({ ...prev, profession: e.target.value }))}
              className="w-full bg-[#1a0938] border border-white/10 rounded-lg px-4 py-2.5 text-white
        ...
        {/* Bio */}
        <div>
          <label className="text-sm text-white/80 mb-1.5 block">Bio</label>
          <textarea
            value={formData.bio}
            onChange={e => setFormData(prev => ({ ...prev, bio: e.target.value }))}
            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40
              focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all h-20 resize-none"
        ...
        {/* Interests */}
        <div>
          <label className="text-sm text-white/80 mb-1.5 block">Interests (Select up to 5) *</label>
        ...
        {/* Social Links - Remove scrolling */}
        <div className="grid grid-cols-1 gap-3 mt-4">
          {['twitter', 'github', 'discord'].map(platform => (
            <div key={platform} className="flex items-center gap-2">
              <span className="w-16 text-sm text-white/60 capitalize">{platform}</span>
              <input
                type="text"
